...
<template>
  <div class="min-h-screen w-full flex bg-gradient-to-tr from-surface-container-low via-surface-bright to-rose-500/10 text-on-background antialiased font-body-md text-body-md select-none">
    <!-- Split Screen Layout -->
    <div class="flex w-full min-h-screen">
      <!-- Left Side: Image/Brand (Hidden on Mobile) -->
      <div class="hidden lg:flex w-1/2 flex-col justify-between relative overflow-hidden" style="background-color: #881337;">
        ...
        ></div>
        <!-- Overlaid Premium Emergency Crimson Gradient -->
        <div class="absolute inset-0" style="background: linear-gradient(to bottom right, rgba(159, 18, 57, 0.75), rgba(136, 19, 55, 0.85));"></div>
        <div class="absolute inset-0" style="background: linear-gradient(to top, rgba(15,23,42,0.45), transparent);"></div>
        ...
        <div class="relative z-10 p-xl flex flex-col h-full justify-between">
          ...
          <div class="flex flex-col gap-0.5">
            ...
            <p class="font-label-md text-label-md text-rose-200 tracking-widest uppercase mt-1">NTPD KABUPATEN BULUKUMBA</p>
          </div>
          <!-- Mission Statement -->
          <div class="max-w-md">
...
Layanan Presensi Siaga.</h2>
            <p class="font-body-md text-body-md text-rose-100 opacity-90 leading-relaxed">Sistem informasi manajemen presensi terintegrasi dengan pengawasan lokasi presisi geofence radius 2 meter dan autentikasi aman untuk seluruh Call Taker NTPD 112 Kabupaten Bulukumba.</p>
          </div>
          <!-- Footer Branding -->
          <div class="font-body-sm text-body-sm text-rose-200">
      ...
      <div class="w-full lg:w-1/2 flex items-center justify-center p-md lg:p-xl bg-gradient-to-tr from-surface-container-low/40 to-surface-bright">
        <div class="w-full max-w-md bg-white/90 backdrop-blur-md rounded-3xl shadow-xl shadow-slate-100/60 border border-outline-variant/20 p-xl flex flex-col gap-lg">
          <!-- Mobile Branding (Visible only on small screens) -->
          <div class="flex lg:hidden flex-col items-center gap-1 mb-sm justify-center text-center">
            <h1 class="font-headline-md text-headline-md text-rose-800">LOPI-Q</h1>
          ...
          <div v-if="showSetup2FA" class="flex flex-col gap-3">
            ...
            <div v-if="setupStep === 1" class="flex flex-col gap-3">
              ...
              <div class="flex items-center gap-4 bg-slate-50 border border-outline-variant/20 p-3 rounded-2xl">
                <!-- QR Code Container -->
                <div class="relative w-28 h-28 flex items-center justify-center border border-outline-variant/50 p-1.5 rounded-xl shadow-inner bg-white shrink-0">
                  ...
                  <div v-else class="animate-pulse flex flex-col items-center justify-center gap-1.5 text-on-surface-variant/40">
                    <span class="material-symbols-outlined text-[28px] animate-spin">sync</span>
                ...
                <div class="flex flex-col text-left justify-center min-w-0">
                  ...
                  <div class="bg-white p-2 rounded-lg border border-outline-variant/20 mt-2">
                    <p class="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider leading-none">Kunci Manual (Secret Key)</p>
                    <code class="text-[10px] font-bold text-rose-700 break-all select-all font-mono block mt-1">{{ totpSecret || 'Memuat...' }}</code>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-col gap-2 mt-1">
                <button 
                  ...
                  class="w-full py-2.5 bg-gradient-to-r from-rose-700 to-amber-600 text-white font-bold text-xs rounded-xl hover:shadow-lg hover:shadow-rose-700/20 transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 shadow-sm disabled:opacity-50 duration-200 cursor-pointer border-0" 
                  type="button"
                ...
                <button 
                  ...
                  class="w-full py-2 text-center text-xs text-slate-500 hover:text-on-surface transition-colors bg-transparent border-0 cursor-pointer"
                >
                  Batal & Kembali
                </button>
              </div>
            ...
            <div v-else class="flex flex-col gap-3">
              <div class="p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-[11px] flex items-start gap-2 shadow-sm">
                <span class="material-symbols-outlined text-[18px] shrink-0 mt-0.5 text-amber-600">warning</span>
                <div>
              ...
              <!-- Backup codes grid -->
              <div class="grid grid-cols-2 gap-2 bg-slate-100 text-slate-800 p-3 rounded-xl font-mono text-xs font-bold tracking-wider text-center border border-slate-200 shadow-inner">
                <div v-for="c in backupCodes" :key="c" class="py-1 px-2 border-b border-slate-200 hover:text-rose-700 transition-colors">
                  {{ c }}
                </div>
              </div>
              ...
              <div class="flex gap-2">
                <button 
                  ...
                  class="flex-1 py-2 px-3 border border-outline-variant hover:bg-slate-50 transition-colors text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 cursor-pointer bg-white"
                  type="button"
                >
              ...
              <button 
                ...
                class="w-full py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold text-xs rounded-xl hover:shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 shadow-sm duration-200 cursor-pointer border-0" 
                type="button"
              >
          ...
          <div v-else class="flex flex-col gap-lg">
            ...
            <transition name="fade">
              <div v-if="successMessage" class="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-body-sm flex items-start gap-3 shadow-sm">
                <span class="material-symbols-outlined text-[20px] shrink-0 mt-0.5 text-emerald-600">check_circle</span>
                <div>
            ...
            <form v-if="!authStore.otpRequired" @submit.prevent="handleLogin" class="flex flex-col gap-md">
              ...
              <div class="flex flex-col gap-xs">
                ...
                <div class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant text-[20px]">person</span>
                  <input 
                    v-model="email"
                    class="w-full pl-11 pr-4 py-3 bg-surface-container-low/50 border border-outline-variant/50 focus:border-rose-600 focus:ring-4 focus:ring-rose-500/10 rounded-xl focus:outline-none transition-all font-body-sm text-body-sm text-on-surface placeholder:text-on-surface-variant/40" 
                    id="email" 
                    name="email" 
              ...
              <div class="flex flex-col gap-xs">
                ...
                <div class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant text-[20px]">lock</span>
                  <input 
                    v-model="password"
                    class="w-full pl-11 pr-11 py-3 bg-surface-container-low/50 border border-outline-variant/50 focus:border-rose-600 focus:ring-4 focus:ring-rose-500/10 rounded-xl focus:outline-none transition-all font-body-sm text-body-sm text-on-surface placeholder:text-on-surface-variant/40" 
                    id="password" 
                    name="password" 
                    placeholder="••••••••" 
                    required 
                  ...
                  />
                  <button 
                    class="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-rose-700 transition-colors focus:outline-none flex items-center justify-center border-0 bg-transparent cursor-pointer" 
              ...
              <div class="flex items-center justify-between mt-xs">
                <div class="flex items-center gap-2">
                  <input 
                    class="w-4 h-4 text-rose-600 border-outline-variant rounded focus:ring-rose-500 focus:ring-offset-0 bg-surface cursor-pointer" 
                    id="remember" 
                    name="remember" 
                    type="checkbox"
                  />
                ...
                <button 
                  ...
                  class="font-label-md text-label-md text-rose-700 hover:text-rose-800 transition-colors bg-transparent border-0 cursor-pointer"
                >
                  Lupa Kata Sandi?
                </button>
              </div>
              ...
              <!-- Submit Button -->
              <button 
                class="mt-sm w-full py-3 px-4 bg-gradient-to-r from-rose-700 via-rose-600 to-amber-600 text-white font-label-md text-label-md rounded-xl hover:shadow-lg hover:shadow-rose-900/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-sm disabled:opacity-50 disabled:scale-100 duration-200 cursor-pointer border-0" 
                type="submit"
                :disabled="loading"
              >
            ...
            <form v-else @submit.prevent="handleVerify2fa" class="flex flex-col gap-md">
              <div class="flex flex-col gap-xs">
                ...
                <div v-else class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant text-[20px]">
                    key
                  </span>
                  <input 
                    v-model="otpCode"
              ...
              <!-- Verify/Reset Button -->
              <button 
                class="mt-sm w-full py-3 px-4 bg-gradient-to-r from-rose-700 via-rose-600 to-amber-600 text-white font-label-md text-label-md rounded-xl hover:shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-sm disabled:opacity-50 disabled:scale-100 duration-200 cursor-pointer border-0" 
                type="submit"
                :disabled="loading"
              >
                <svg v-if="loading" class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              <button 
                ...
                class="w-full py-2.5 text-center text-xs text-on-surface-variant hover:text-on-surface transition-colors bg-transparent border-0 cursor-pointer"
              >
          ...
          <div class="mt-auto pt-lg border-t border-outline-variant/30 flex flex-col gap-3 text-center text-[11px]">
            <p class="text-on-surface-variant/70 leading-relaxed font-medium">
              ...
              Developed by <a href="https://diskominfo.bulukumbakab.go.id" target="_blank" rel="noopener noreferrer" class="font-bold text-rose-700 hover:underline">Diskominfo dan Persandian Kab. Bulukumba</a>.
            </p>
            <div class="flex justify-center mt-1">
              <router-link class="px-4 py-2 rounded-full border border-outline-variant/40 hover:border-rose-500 hover:bg-rose-50 text-on-surface-variant hover:text-rose-800 transition-all duration-200 inline-flex items-center justify-center gap-1.5 font-label-md text-xs shadow-sm bg-white/50 cursor-pointer decoration-none" to="/">
