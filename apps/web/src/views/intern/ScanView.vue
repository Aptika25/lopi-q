<template>
  <InternLayout>
    <div class="min-h-screen flex flex-col font-sans text-[#1b1c1c] pb-safe relative select-none">
      ...
      <!-- ===== MAIN CONTENT AREA ===== -->
      <main class="flex-grow flex flex-col items-center justify-center px-4 py-6 sm:py-8 max-w-lg mx-auto w-full space-y-6">
        ...
        <div class="text-center space-y-1">
          ...
          <p class="text-xs sm:text-sm text-[#574146]">
            Posisikan QR Code Posko Siaga 112 tepat di dalam bingkai kamera scanner.
          </p>
        </div>

        <!-- The Scanner Window Container -->
        <div class="relative w-64 h-64 sm:w-72 sm:h-72 rounded-3xl overflow-hidden shadow-[0px_10px_30px_rgba(240,98,146,0.15)] border border-[#ddbfc5] bg-white flex items-center justify-center p-4">
          ...
          <div v-if="cameraActive" class="absolute w-[80%] h-0.5 bg-[#f06292] rounded-full shadow-[0_0_12px_#f06292] scan-line z-20"></div>

          <!-- Fallback Inactive Background -->
          <div v-if="!cameraActive" class="flex flex-col items-center justify-center text-center space-y-2 relative z-10 p-4">
            <span class="material-symbols-outlined text-5xl text-[#ddbfc5]">qr_code</span>
            <p class="text-xs text-[#574146] font-medium">Kamera belum aktif. Klik tombol shutter di bawah.</p>
          </div>
        </div>
