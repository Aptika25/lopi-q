<template>
  <div class="min-h-screen bg-slate-50 text-slate-800 antialiased font-body flex select-none relative overflow-x-hidden pb-24 lg:pb-0">
    ...
    <aside class="hidden lg:flex fixed inset-y-0 left-0 w-64 h-screen bg-white border-r border-slate-200/80 z-50 flex-col justify-between py-5 px-3 lg:sticky lg:top-0 lg:h-screen lg:relative shrink-0">
      ...
      <div class="px-3 mb-6">
        <router-link to="/intern/dashboard" class="flex items-center space-x-3 text-slate-900 decoration-none">
          ...
          <div>
            <h1 class="text-base font-display font-black text-slate-900 tracking-tight leading-none">LOPI<span class="text-primary">-Q</span></h1>
            <p class="text-[9px] font-bold text-primary tracking-wider uppercase mt-1">PORTAL PESERTA MAGANG</p>
          </div>
        </router-link>
      </div>
      ...
      <nav class="flex-grow flex flex-col gap-1 overflow-y-auto">
        ...
        <div v-for="item in menuItems" :key="item.to" class="relative">
          ...
          >
            <span :class="['material-symbols-outlined text-[18px]', $route.path === item.to ? 'fill text-primary' : '']">{{ item.icon }}</span>
    ...
    <div class="flex-grow flex flex-col min-w-0 h-screen overflow-y-auto relative bg-slate-50">
      ...
      <!-- ===== MAIN CONTENT SLOT ===== -->
      <main class="p-4 sm:p-6 flex-grow flex flex-col gap-6 max-w-7xl w-full mx-auto">
        <slot />
      </main>
    ...
    <!-- ========== BOTTOM NAVIGATION BAR (Mobile / Smartphone only) ========== -->
    <nav class="lg:hidden fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-xl border-t border-slate-200/80 z-40 px-2 py-1 shadow-lg select-none">
      <div class="flex items-center justify-around max-w-md mx-auto relative">
        ...
        >
          <div
            :class="[
              ...
              $route.path === '/intern/dashboard' ? 'bg-primary-fixed/40 text-primary' : 'text-slate-400 group-hover:text-slate-600'
            ]"
          >
            <span :class="['material-symbols-outlined text-[22px]', $route.path === '/intern/dashboard' ? 'fill text-primary' : '']">space_dashboard</span>
          </div>
          <span
            :class="[
              'text-[10px] font-bold tracking-tight transition-colors',
              $route.path === '/intern/dashboard' ? 'text-primary font-extrabold' : 'text-slate-400'
            ]"
          >Beranda</span>
        </router-link>
        ...
        >
          <div
            :class="[
              ...
              $route.path === '/intern/scan'
                ? 'bg-gradient-to-tr from-primary via-primary-container to-secondary text-on-primary shadow-primary/50 scale-110 ring-4 ring-primary/25'
                : 'bg-gradient-to-tr from-primary to-secondary text-on-primary shadow-primary/40 hover:scale-105'
            ]"
          >
          ...
          <span
            :class="[
              ...
              $route.path === '/intern/scan' ? 'text-primary font-black' : 'text-slate-700 font-extrabold'
            ]"
          >Absensi</span>
        </router-link>
        ...
        >
          <div
            :class="[
              'w-10 h-8 rounded-xl flex items-center justify-center transition-all duration-200',
              $route.path === '/intern/history' ? 'bg-primary-fixed/40 text-primary' : 'text-slate-400 group-hover:text-slate-600'
            ]"
          >
            <span :class="['material-symbols-outlined text-[22px]', $route.path === '/intern/history' ? 'fill text-primary' : '']">history_edu</span>
          </div>
          <span
            :class="[
              'text-[10px] font-bold tracking-tight transition-colors',
              $route.path === '/intern/history' ? 'text-primary font-extrabold' : 'text-slate-400'
            ]"
          >Riwayat</span>
        </router-link>
...
const menuItems = [
  { to: '/intern/dashboard', label: 'Beranda Status Presensi', icon: 'space_dashboard' },
  { to: '/intern/scan', label: 'Absensi & Scan QR Code', icon: 'qr_code_scanner' },
  { to: '/intern/history', label: 'Riwayat Presensi Saya', icon: 'history_edu' }
];

const navItems = [
  { to: '/intern/dashboard', label: 'Beranda', icon: 'space_dashboard' },
  { to: '/intern/scan', label: 'Absensi', icon: 'qr_code_scanner' },
  { to: '/intern/history', label: 'Riwayat', icon: 'history_edu' }
];

const pageTitle = computed(() => {
  const path = route.path;
  if (path === '/intern/history') return 'Riwayat Presensi Saya';
  if (path === '/intern/scan') return 'Absensi & Scan QR Code';
  return 'Beranda Status Presensi';
});

const breadcrumbSub = computed(() => {
  const path = route.path;
  if (path === '/intern/history') return 'Riwayat Presensi';
  if (path === '/intern/scan') return 'Absensi QR';
  return 'Beranda';
});
