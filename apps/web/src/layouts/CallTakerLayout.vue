<template>
  <div class="min-h-screen bg-slate-50 text-slate-800 antialiased font-body flex select-none relative overflow-x-hidden pb-24 lg:pb-0">
    ...
    <aside class="hidden lg:flex fixed inset-y-0 left-0 w-64 h-screen bg-white border-r border-slate-200/80 z-50 flex-col justify-between py-5 px-3 lg:sticky lg:top-0 lg:h-screen lg:relative shrink-0">
      ...
      <div class="px-3 mb-6">
        <router-link to="/calltaker/dashboard" class="flex items-center space-x-3 text-slate-900 decoration-none">
          ...
          <div>
            ...
            <p class="text-[9px] font-bold text-rose-800 tracking-wider uppercase mt-1">CALL TAKER PANEL</p>
          </div>
        </router-link>
      </div>
      ...
      <div class="border-t border-slate-200/80 pt-3.5 flex flex-col gap-3">
        <div class="flex items-center gap-2.5 px-3">
          ...
          <div class="flex flex-col justify-center min-w-0 text-left">
            <span class="text-xs font-bold text-slate-900 truncate leading-tight block">{{ authStore.user?.name || 'Call Taker' }}</span>
            <span class="text-[9px] text-slate-400 font-mono uppercase truncate leading-none mt-0.5 block">CALL TAKER</span>
          </div>
        </div>
    ...
    <div class="flex-grow flex flex-col min-w-0 h-screen overflow-y-auto relative bg-slate-50">
      ...
      <header class="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/60 shadow-xs min-h-14">
        ...
        <div class="lg:hidden flex items-center justify-between px-4 h-14">
          <!-- Logo LOPI-Q -->
          <router-link to="/calltaker/dashboard" class="flex items-center gap-2.5 decoration-none">
            ...
            <div>
              ...
              <div class="text-[8px] font-bold text-rose-800 tracking-widest uppercase leading-none mt-0.5">CALL TAKER PANEL</div>
            </div>
          </router-link>
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
      ...
      <div class="relative flex items-center justify-around px-4 pt-1.5 pb-safe-offset-2" style="padding-bottom: max(0.6rem, env(safe-area-inset-bottom))">
        ...
        >
          <div
            :class="[
              ...
              $route.path === '/calltaker/dashboard' ? 'bg-rose-50 text-rose-700' : 'text-slate-400 group-hover:text-slate-600'
            ]"
          >
            <span :class="['material-symbols-outlined text-[22px]', $route.path === '/calltaker/dashboard' ? 'fill text-rose-700' : '']">grid_view</span>
          </div>
          <span
            :class="[
              'text-[10px] font-bold tracking-tight transition-colors',
              $route.path === '/calltaker/dashboard' ? 'text-rose-700 font-extrabold' : 'text-slate-400'
            ]"
          >Dashboard</span>
        </router-link>
        ...
        >
          ...
          <span
            :class="[
              ...
              $route.path === '/calltaker/scan' ? 'text-rose-700 font-black' : 'text-slate-700 font-extrabold'
            ]"
          >Scan QR</span>
        </router-link>
        ...
        <router-link
          to="/calltaker/history"
          class="flex flex-col items-center gap-0.5 flex-1 py-1 decoration-none group transition-all duration-200"
        >
          <div
            :class="[
              'w-10 h-8 rounded-xl flex items-center justify-center transition-all duration-200',
              $route.path === '/calltaker/history' ? 'bg-rose-50 text-rose-700' : 'text-slate-400 group-hover:text-slate-600'
            ]"
          >
            <span :class="['material-symbols-outlined text-[22px]', $route.path === '/calltaker/history' ? 'fill text-rose-700' : '']">history_edu</span>
          </div>
          <span
            :class="[
              'text-[10px] font-bold tracking-tight transition-colors',
              $route.path === '/calltaker/history' ? 'text-rose-700 font-extrabold' : 'text-slate-400'
            ]"
          >Riwayat</span>
        </router-link>
...
const menuItems = [
  { to: '/calltaker/dashboard', label: 'Dashboard Status Siaga', icon: 'dashboard' },
  { to: '/calltaker/scan', label: 'Scan QR Presensi', icon: 'qr_code_scanner' },
  { to: '/calltaker/history', label: 'Laporan Kehadiran Saya', icon: 'description' }
];

const navItems = [
  { to: '/calltaker/dashboard', label: 'Dashboard', icon: 'grid_view' },
  { to: '/calltaker/scan', label: 'Scan QR', icon: 'qr_code_scanner' },
  { to: '/calltaker/history', label: 'Riwayat', icon: 'description' }
];

const pageTitle = computed(() => {
  const path = route.path;
  if (path === '/calltaker/history') return 'Laporan Kehadiran Saya';
  if (path === '/calltaker/scan') return 'Scan QR Presensi';
  return 'Dashboard Status Siaga';
});

const breadcrumbSub = computed(() => {
  const path = route.path;
  if (path === '/calltaker/history') return 'Riwayat Presensi';
  if (path === '/calltaker/scan') return 'Scan QR';
  return 'Dashboard';
});
